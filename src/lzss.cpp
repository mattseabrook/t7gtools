...
/*
    ...
    LZSS Decompression Algorithm implementation specifically for The 7th Guest
...
{
    uint16_t N = 1 << (0xf - lengthBits + 1);
    uint16_t F = 1 << lengthBits;
    int Threshold = 3;
    uint8_t Flags = 0;
    uint8_t b = 0;
    uint8_t i = 0;
    ...
    uint16_t historyBufferPosition = N - F;
        ...
        for (i = 0; i < 8; i++)
        {
            if (inputBufferPosition < blockSize - 1)
            {
                if ((Flags & 1) == 0)
                {
                    ...
                    uint16_t Length = (OfsLen & ((1 << lengthBits) - 1)) + Threshold;
                    uint16_t Offset = (historyBufferPosition - (OfsLen >> lengthBits)) & (N - 1);
