...
uses
  SysUtils, Classes, StrUtils, SaveBMP2, AviWriter, Graphics;
...
// 2023 NEW PROCEDURE
procedure WriteBinaryFile(const Filename: string; const Buffer: TBuffer);
var
...
begin
  ...
  BlockIndex := 0; // 2023 - Initialize the block index
  ...
  begin
    ...
    // 2023 changes
    //    - added begin/end around the if statement
    //    - We are writing the contents of OutBuf to the HDD
    //      at each permutation:
    //        - uncompressed
    //        - compressed
    //        - decompressed
    ...
    begin
      OutBuf := Copy(InBuf);
      // uncompressed block
      WriteBinaryFile(Format('%s-%.2d-%d(uncompressed).bin', [VDXName, BlockIndex, BlockHeader.PlayCmd]), OutBuf);
    end
    else
    begin
      // compressed block
      WriteBinaryFile(Format('%s-%.2d-%d(compressed).bin', [VDXName, BlockIndex, BlockHeader.PlayCmd]), OutBuf);

      DecompressBlock(InBuf, BlockHeader, OutBuf);

      // decompressed block
      WriteBinaryFile(Format('%s-%.2d-%d(decompressed).bin', [VDXName, BlockIndex, BlockHeader.PlayCmd]), OutBuf);
    end;
    ...
    Inc(BlockIndex); // 2023 - Increment the block index
  end;
...
begin
  Writeln('VDXExt 2023: Debug version');
  Writeln('Matt Seabrook'#10);
